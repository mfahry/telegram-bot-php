	...
	public function webhook() {
		...
		if($this->telegram->Text() != null) {
			...
			if($command_checker[0] == "/request" || $command_checker[0] == "/request@customer_care_bot") {
				...
				if(count($command_checker) == 2) {
					...
					if($user_accepted_request != null){
						...
						if($pushchat != null){
							...
							$content = array(
								"chat_id" => $chat_id,
								"text" => "Maaf pushchat masih aktif untuk nomor tiket ".$pushchat->TICKET_ID.". Silahkan tutup terlebih dahulu dengan <code>/endpushchat</code> command",
								"reply_to_message_id" => $message_id,
								"parse_mode" => "HTML"
							);
				...
				$text = "Tiket dengan status OPEN terkait user anda :".chr(10).chr(10);
				...
				if($user_accepted_request != null) {
					...
					if($pushchat != null){
						...
						if($user_accepted_request->QUOTA_DESCRIPTION < 5) {
							...
							$this->telegram_message_model->change($data, $where);
				...
				/*else {
					$content = array(
						"chat_id" => $chat_id,
						"text" => "Maaf, tidak ada tiket yang OPEN untuk user ini.",
						"reply_to_message_id" => $message_id,
						"parse_mode" => "HTML"
					);
				}*/
